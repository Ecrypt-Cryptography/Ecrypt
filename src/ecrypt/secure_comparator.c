 ...
 * paper with SMP solution and our GitHub issues https://github.com/cossacklabs/ecrypt/issues/85 and
 * https://github.com/cossacklabs/ecrypt/issues/83.
 *
...
#include "ecrypt/secure_comparator.h"

#include <string.h>

#include "ecconnect/ecconnect_wipe.h"

#include "ecrypt/secure_comparator_t.h"

static ecrypt_status_t secure_comparator_alice_step1(secure_comparator_t* comp_ctx,
                                                     void* output,
                                                     size_t* output_length);
static ecrypt_status_t secure_comparator_bob_step2(secure_comparator_t* comp_ctx,
                                                   const void* input,
                                                   size_t input_length,
                                                   void* output,
                                                   size_t* output_length);
static ecrypt_status_t secure_comparator_alice_step3(secure_comparator_t* comp_ctx,
                                                     const void* input,
                                                     size_t input_length,
                                                     void* output,
                                                     size_t* output_length);
static ecrypt_status_t secure_comparator_bob_step4(secure_comparator_t* comp_ctx,
                                                   const void* input,
                                                   size_t input_length,
                                                   void* output,
                                                   size_t* output_length);
static ecrypt_status_t secure_comparator_alice_step5(secure_comparator_t* comp_ctx,
                                                     const void* input,
                                                     size_t input_length,
                                                     void* output,
                                                     size_t* output_length);
...
static ecrypt_status_t ed_sign(uint8_t pos, const uint8_t* scalar, uint8_t* signature)
{
    ...
    res = ecconnect_hash_init(&hash_ctx, ECCONNECT_HASH_SHA512);
    if (ECRYPT_SUCCESS != res) {
        return res;
    }

    res = ecconnect_hash_update(&hash_ctx, k, ED25519_GE_LENGTH);
    if (ECRYPT_SUCCESS != res) {
        ecconnect_hash_cleanup(&hash_ctx);
        return res;
    }

    res = ecconnect_hash_update(&hash_ctx, &pos, sizeof(pos));
    if (ECRYPT_SUCCESS != res) {
        ecconnect_hash_cleanup(&hash_ctx);
        return res;
    }

    res = ecconnect_hash_final(&hash_ctx, k, &hash_length);
...
static ecrypt_status_t ed_verify(uint8_t pos, const ge_p3* point, const uint8_t* signature)
{
    ge_p3 A;
    ge_p2 R;
...
static ecrypt_status_t ed_dbl_base_sign(uint8_t pos,
...
static ecrypt_status_t ed_dbl_base_verify(uint8_t pos,
                                          const ge_p3* base1,
                                          const ge_p3* base2,
                                          const ge_p3* point1,
                                          const ge_p3* point2,
...
static ecrypt_status_t ed_point_sign(uint8_t pos, const uint8_t* scalar, const ge_p3* point, uint8_t* signature)
{
    uint8_t r[ED25519_GE_LENGTH];
    ge_p3 R;
    uint8_t k[TEMP_BUFFER_LENGTH];
...
static ecrypt_status_t ed_point_verify(uint8_t pos,
                                       const ge_p3* base2,
                                       const ge_p3* point1,
                                       const ge_p3* point2,
                                       const uint8_t* signature)
...
ecrypt_status_t secure_comparator_init(secure_comparator_t* comp_ctx)
{
    ...
    ecconnect_status = ecconnect_hash_init(&(comp_ctx->hash_ctx), ECCONNECT_HASH_SHA256);
    if (ECCONNECT_SUCCESS != ecconnect_status) {
        return (ecrypt_status_t)ecconnect_status;
    }

    return ECRYPT_SUCCESS;
}

ecrypt_status_t secure_comparator_cleanup(secure_comparator_t* comp_ctx)
{
    if (!comp_ctx) {
        return ECRYPT_INVALID_PARAMETER;
    }

    ecconnect_hash_cleanup(&(comp_ctx->hash_ctx));

    ecconnect_wipe(comp_ctx, sizeof(secure_comparator_t));
...
{
    ecrypt_status_t res = secure_comparator_cleanup(comp_ctx);
    if (ECRYPT_SUCCESS == res) {
        free(comp_ctx);
    }
                                                ...
                                                size_t secret_data_length)
{
    ...
    ecconnect_status = ecconnect_hash_update(&(comp_ctx->hash_ctx), secret_data, secret_data_length);
    return (ecrypt_status_t)ecconnect_status;
}
                                                     ...
                                                     size_t* output_length)
{
    ...
    *output_length = 6 * ED25519_GE_LENGTH;
    ...
    ecrypt_status = (ecrypt_status_t)ecconnect_hash_final(&(comp_ctx->hash_ctx),
                                                      comp_ctx->secret,
                                                      &secret_length);
    if (ECRYPT_SUCCESS != ecrypt_status) {
        return ecrypt_status;
    }
    ...
    ecrypt_status = ed_sign(1, comp_ctx->rand2, ((unsigned char*)output) + ED25519_GE_LENGTH);
    if (ECRYPT_SUCCESS != ecrypt_status) {
        return ecrypt_status;
    }
    ...
    ecrypt_status = ed_sign(2, comp_ctx->rand3, ((unsigned char*)output) + (4 * ED25519_GE_LENGTH));
    if (ECRYPT_SUCCESS != ecrypt_status) {
        return ecrypt_status;
    }
    ...
    return ECRYPT_SCOMPARE_SEND_OUTPUT_TO_PEER;
}
                                                   ...
                                                   size_t* output_length)
{
    ...
    ecrypt_status = ed_verify(1, &g2a, (const unsigned char*)input + ED25519_GE_LENGTH);
    if (ECRYPT_INVALID_SIGNATURE == ecrypt_status) {
        comp_ctx->result = ECRYPT_SCOMPARE_NO_MATCH;
    } else if (ECRYPT_SUCCESS != ecrypt_status) {
        return ecrypt_status;
    }

    /* Verification of G2b signature */
    ecrypt_status = ed_verify(2, &(comp_ctx->g3p), (const unsigned char*)input + (4 * ED25519_GE_LENGTH));
    if (ECRYPT_INVALID_SIGNATURE == ecrypt_status) {
        comp_ctx->result = ECRYPT_SCOMPARE_NO_MATCH;
    } else if (ECRYPT_SUCCESS != ecrypt_status) {
        return ecrypt_status;
    ...
    *output_length = 11 * ED25519_GE_LENGTH;
    ...
    ecrypt_status = ed_sign(3, comp_ctx->rand2, ((unsigned char*)output) + ED25519_GE_LENGTH);
    if (ECRYPT_SUCCESS != ecrypt_status) {
        return ecrypt_status;
    }
    ...
    ecrypt_status = ed_sign(4, comp_ctx->rand3, ((unsigned char*)output) + (4 * ED25519_GE_LENGTH));
    if (ECRYPT_SUCCESS != ecrypt_status) {
        return ecrypt_status;
    }
    ...
    ecrypt_status = ed_dbl_base_sign(5,
                                     comp_ctx->rand,
                                     comp_ctx->secret,
                                     &(comp_ctx->g2),
                                     &(comp_ctx->g3),
                                                     ...
                                                     size_t* output_length)
{
    ...
    /* Verification of G2b signature */
    ecrypt_status = ed_verify(3, &g2b, (const unsigned char*)input + ED25519_GE_LENGTH);
    if (ECRYPT_INVALID_SIGNATURE == ecrypt_status) {
        comp_ctx->result = ECRYPT_SCOMPARE_NO_MATCH;
    } else if (ECRYPT_SUCCESS != ecrypt_status) {
        return ecrypt_status;
    ...
    ecrypt_status = ed_verify(4, &(comp_ctx->g3p), (const unsigned char*)input + (4 * ED25519_GE_LENGTH));
    if (ECRYPT_INVALID_SIGNATURE == ecrypt_status) {
        comp_ctx->result = ECRYPT_SCOMPARE_NO_MATCH;
    } else if (ECRYPT_SUCCESS != ecrypt_status) {
        return ecrypt_status;
    ...
    *output_length = 8 * ED25519_GE_LENGTH;
    ...
    ecrypt_status = ed_dbl_base_verify(5,
                                       &(comp_ctx->g2),
                                       &(comp_ctx->g3),
                                       &(comp_ctx->Pp),
                                       &Qb,
    ...
    ecrypt_status = ed_dbl_base_sign(6,
                                     comp_ctx->rand,
                                     comp_ctx->secret,
                                     &(comp_ctx->g2),
                                     &(comp_ctx->g3),
                                                   ...
                                                   size_t* output_length)
{
    ...
    *output_length = 3 * ED25519_GE_LENGTH;
    ...
    ecrypt_status = ed_dbl_base_verify(6,
                                       &(comp_ctx->g2),
                                       &(comp_ctx->g3),
                                       &Pa,
                                       &Qa,
    ...
    ecrypt_status = ed_point_verify(7,
                                    &(comp_ctx->g3p),
                                    &Qa,
                                    &Ra,
    ...
    /* Bob finishes */
    if (ECRYPT_SCOMPARE_NOT_READY == comp_ctx->result) {
        comp_ctx->result = ge_cmp(&Rab, &Pa_Pb) ? ECRYPT_SCOMPARE_NO_MATCH : ECRYPT_SCOMPARE_MATCH;
    }
    ...
    ecrypt_status = ed_point_sign(8, comp_ctx->rand3, &Qa, ((unsigned char*)output) + ED25519_GE_LENGTH);
    if (ECRYPT_SUCCESS != ecrypt_status) {
        return ecrypt_status;
    }

    ecconnect_wipe(comp_ctx->secret, sizeof(comp_ctx->secret));
                                                     ...
                                                     size_t* output_length)
{
    ...
    *output_length = 0;
    ...
    ecrypt_status = ed_point_verify(8,
                                    &(comp_ctx->g3p),
                                    &(comp_ctx->Qa_Qb),
                                    &Rb,
