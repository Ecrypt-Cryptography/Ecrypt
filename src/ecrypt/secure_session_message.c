...
#define ECRYPT_SESSION_WRAP_TAG "TSWM"
...
ecrypt_status_t secure_session_wrap(secure_session_t* session_ctx,
                                    const void* message,
                                    ...
                                    size_t* wrapped_message_length)
{
    ...
    *length = htobe32(message_length + sizeof(uint32_t) + sizeof(uint64_t));
    ...
    res = ecconnect_rand(iv, CIPHER_MAX_BLOCK_SIZE);
...
ecrypt_status_t secure_session_unwrap(secure_session_t* session_ctx,
                                      ...
                                      size_t* message_length)
{
    ...
    if (!secure_session_is_established(session_ctx)) {
        res = session_ctx->state_handler(session_ctx,
                                         ...
                                         message,
                                         message_length);
        if ((ECRYPT_SUCCESS == res) && (*message_length > 0)) {
            return ECRYPT_SSESSION_SEND_OUTPUT_TO_PEER;
        }
    ...
    sym_ctx = ecconnect_sym_aead_decrypt_create(ECCONNECT_SYM_AES_GCM | ECCONNECT_SYM_256_KEY_LENGTH,
    ...
    res = ecconnect_sym_aead_decrypt_update(sym_ctx,
    ...
    *message_length = length - (sizeof(uint32_t) + sizeof(uint64_t));
    ...
    res = ecconnect_sym_aead_decrypt_final(sym_ctx,
...
err:
    ...
    if (NULL != sym_ctx) {
        ecconnect_sym_aead_decrypt_destroy(sym_ctx);
    }
