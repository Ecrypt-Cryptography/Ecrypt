...
#ifndef ECRYPT_SECURE_CELL_SEAL_PASSPHRASE_H
#define ECRYPT_SECURE_CELL_SEAL_PASSPHRASE_H
 ...
 *
 * Currently Ecrypt supports only one KDF -- PBKDF2 -- which has the following
 * context layout:
 *
 * ```
 *     0        1        2        3        4        5        6        7
 ...
 * ```
 *
 * @see Ecrypt RFC 2 contains detailed reference on data formats.
 */
 ...
 */
struct ecrypt_scell_auth_token_passphrase {
    uint32_t alg;
    const uint8_t* iv;
    uint32_t iv_length;
...
static const uint64_t ecrypt_scell_auth_token_passphrase_min_size = 5 * sizeof(uint32_t);

static inline uint64_t ecrypt_scell_auth_token_passphrase_size(
 ...
 */
static inline ecrypt_status_t ecrypt_write_scell_auth_token_passphrase(
    const struct ecrypt_scell_auth_token_passphrase* hdr, uint8_t* buffer, size_t buffer_length)
{
    if (buffer_length < ecrypt_scell_auth_token_passphrase_size(hdr)) {
 ...
 */
static inline ecrypt_status_t ecrypt_read_scell_auth_token_passphrase(
    const uint8_t* buffer, size_t buffer_length, struct ecrypt_scell_auth_token_passphrase* hdr)
{
    uint64_t need_length = ecrypt_scell_auth_token_passphrase_min_size;
...
static inline ecrypt_status_t ecrypt_scell_auth_token_message_size(const uint8_t* auth_token,
                                                                   size_t auth_token_length,
                                                                   uint32_t* message_length)
{
    ECRYPT_CHECK_PARAM(message_length != NULL);
    if (auth_token_length < ecrypt_scell_auth_token_passphrase_min_size) {
        return ECRYPT_FAIL;
    }
 ...
 * It does **not** represent actual in-memory data layout.
 */
struct ecrypt_scell_pbkdf2_context {
    uint32_t iteration_count;
    const uint8_t* salt;
    uint16_t salt_length;
};

static const uint32_t ecrypt_scell_pbkdf2_context_min_size = sizeof(uint32_t) + sizeof(uint16_t);

static inline uint32_t ecrypt_scell_pbkdf2_context_size(const struct ecrypt_scell_pbkdf2_context* ctx)
{
...
static inline ecrypt_status_t ecrypt_write_scell_pbkdf2_context(
    const struct ecrypt_scell_auth_token_passphrase* hdr,
    const struct ecrypt_scell_pbkdf2_context* ctx,
    uint8_t* buffer,
    size_t buffer_length)
{
    ...
    if (hdr->kdf_context_length != ecrypt_scell_pbkdf2_context_size(ctx)) {
        return ECRYPT_FAIL;
    }
    /* Add separately to avoid overflows in intermediate calculations */
...
static inline ecrypt_status_t ecrypt_read_scell_pbkdf2_context(
    const struct ecrypt_scell_auth_token_passphrase* hdr, struct ecrypt_scell_pbkdf2_context* ctx)
{
    const uint8_t* buffer = hdr->kdf_context;
    size_t buffer_length = hdr->kdf_context_length;
    size_t need_length = ecrypt_scell_pbkdf2_context_min_size;
    if (buffer_length < need_length) {
        return ECRYPT_FAIL;
    }
...
#endif /* ECRYPT_SECURE_CELL_SEAL_PASSPHRASE_H */
