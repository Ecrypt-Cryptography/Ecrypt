...
ifeq ($(CRYPTO_ENGINE_LIB_PATH),)
	...
	ECCONNECT_ENGINE_DEPS += $(BIN_PATH)/boringssl/crypto/libcrypto.a $(BIN_PATH)/boringssl/decrepit/libdecrepit.a
else
	CRYPTO_ENGINE_LDFLAGS += -L$(CRYPTO_ENGINE_LIB_PATH)
endif
CRYPTO_ENGINE_LDFLAGS += -lcrypto -ldecrepit -lpthread

ECCONNECT_ENGINE_CMAKE_FLAGS += -DCMAKE_BUILD_TYPE=Release
ECCONNECT_ENGINE_CMAKE_FLAGS += -DCMAKE_C_FLAGS="-fpic"

ifneq ($(NINJA),)
ECCONNECT_ENGINE_CMAKE_FLAGS += -G Ninja
endif

# Cross-compilation support for macOS
ifdef IS_MACOS
ifdef SDK
ECCONNECT_ENGINE_CMAKE_FLAGS += -DCMAKE_OSX_SYSROOT=$(SDK)
endif
ifdef ARCH
ECCONNECT_ENGINE_CMAKE_FLAGS += -DCMAKE_OSX_ARCHITECTURES=$(ARCH)
endif
endif
...
ECCONNECT_ENGINE_CMAKE_FLAGS += -DOPENSSL_NO_ASM=1
endif

ifeq ($(RENAME_BORINGSSL_SYMBOLS),yes)
# The prefix must be a valid C identifier. Replace invalid characters.
ECCONNECT_BORINGSSL_PREFIX := $(shell echo "ECCONNECT_$(VERSION)" | tr '.-' '_')

ECCONNECT_CRYPTO_ENGINE_CFLAGS += -DBORINGSSL_PREFIX=$(ECCONNECT_BORINGSSL_PREFIX)
ECCONNECT_CRYPTO_ENGINE_CFLAGS += -I$(BIN_PATH)/boringssl/stage-2/symbol_prefix_include
endif
...
$(BIN_PATH)/boringssl/crypto/libcrypto.a $(BIN_PATH)/boringssl/decrepit/libdecrepit.a:
	...
	@cd $(BIN_PATH)/boringssl/stage-1 && \
	 $(CMAKE) $(ECCONNECT_ENGINE_CMAKE_FLAGS) $(abspath third_party/boringssl/src)
ifeq ($(NINJA),)
...
ifeq ($(RENAME_BORINGSSL_SYMBOLS),yes)
	...
	@cd $(BIN_PATH)/boringssl/stage-2 && \
	 $(CMAKE) $(ECCONNECT_ENGINE_CMAKE_FLAGS) \
	     -DBORINGSSL_PREFIX=$(ECCONNECT_BORINGSSL_PREFIX) \
