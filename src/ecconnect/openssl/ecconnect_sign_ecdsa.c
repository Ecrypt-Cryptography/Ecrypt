                                                ...
                                                const size_t public_key_length)
{
    ...
    if ((!private_key) && (!public_key)) {
        err = ecconnect_ec_gen_key(&ctx->pkey);
        ...
        if (private_key != NULL) {
            err = ecconnect_ec_import_key(ctx->pkey, private_key, private_key_length);
            if (err != ECCONNECT_SUCCESS) {
                goto free_pkey;
            }
        }
        if (public_key != NULL) {
            err = ecconnect_ec_import_key(ctx->pkey, public_key, public_key_length);
            if (err != ECCONNECT_SUCCESS) {
                goto free_pkey;
            }
        }
                                                              ...
                                                              size_t* key_length)
{
    ...
    return ecconnect_ec_export_private_key(ctx->pkey, key, key_length);
}
                                                             ...
                                                             size_t* key_length)
{
    ...
    return ecconnect_ec_export_public_key(ctx->pkey, compressed, key, key_length);
}
