...
static size_t ec_pub_key_size(int curve, bool compressed)
{
    ...
    case NID_X9_62_prime256v1: /* P-256 */
        return sizeof(ecconnect_container_hdr_t)
...
static size_t ec_priv_key_size(int curve)
{
    switch (curve) {
    case NID_X9_62_prime256v1: /* P-256 */
        return sizeof(ecconnect_ec_priv_key_256_t);
    case NID_secp384r1: /* P-384 */
        return sizeof(ecconnect_ec_priv_key_384_t);
    case NID_secp521r1: /* P-521 */
        return sizeof(ecconnect_ec_priv_key_521_t);
    default:
        return 0;
    }
}
                                                   ...
                                                   size_t* key_length)
{
    ...
    *key_length = output_length;
    ...
    if ((output_length - sizeof(ecconnect_container_hdr_t))
        != EC_POINT_point2oct(group,
                              Q,
                              compressed ? POINT_CONVERSION_COMPRESSED : POINT_CONVERSION_UNCOMPRESSED,
                              (unsigned char*)(key + 1),
                              output_length - sizeof(ecconnect_container_hdr_t),
                              NULL)) {
        res = ECCONNECT_FAIL;
        goto err;
    }

    memcpy(key->tag, ec_pub_key_tag(curve), ECCONNECT_CONTAINER_TAG_LENGTH);
    key->size = htobe32(output_length);
    ecconnect_update_container_checksum(key);
    *key_length = output_length;
                                                    ...
                                                    size_t* key_length)
{
    ...
    *key_length = output_length;
    ...
    if ((output_length - sizeof(ecconnect_container_hdr_t))
        != bn_encode(d, (unsigned char*)(key + 1), output_length - sizeof(ecconnect_container_hdr_t))) {
        res = ECCONNECT_FAIL;
        goto err;
    }

    memcpy(key->tag, ec_priv_key_tag(curve), ECCONNECT_CONTAINER_TAG_LENGTH);
    key->size = htobe32(output_length);
    ecconnect_update_container_checksum(key);
    *key_length = output_length;
    res = ECCONNECT_SUCCESS;
                                                   ...
                                                   ecconnect_engine_specific_ec_key_t** engine_key)
{
    ...
    if (ECCONNECT_SUCCESS != ecconnect_verify_container_checksum(key)) {
    ...
    if (1
        != EC_POINT_oct2point(group,
                              ...
                              (int)(key_length - sizeof(ecconnect_container_hdr_t)),
                              NULL)) {
        res = ECCONNECT_INVALID_PARAMETER;
        goto err;
    }
                                                    ...
                                                    ecconnect_engine_specific_ec_key_t** engine_key)
{
    ...
    d = BN_bin2bn((const unsigned char*)(key + 1), (int)(key_length - sizeof(ecconnect_container_hdr_t)), NULL);
    if (NULL == d) {
        res = ECCONNECT_NO_MEMORY;
        goto err;
    }
